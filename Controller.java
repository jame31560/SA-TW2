...
public class Controller {
    ...
    public boolean login(String username, String password) {
        user = dbMgr.getUserByUsername(username);//rule 4
        if (user == null) {
            return false;
        } else {
    ...
    public String[] regist(String username,
        ...
        if (username.equals("") || 
                dbMgr.getUserByUsername(username) != null) {//rule 4
        ...
        User registUser = new User(username,
            ...
            balance,
            phone);
        dbMgr.addUser(registUser);//rule 4
    ...
    public String[] changePassword(String password,
        ...
        if (password.equals(user.getPassword())) {
            if (newPassword.equals(confirmPassword)) {
                user.setPassword(newPassword);//rule 2
                return new String[] {"Success"};
            } else {
    ...
    public String[] makeTransaction(User payer, int amount) {//stamp coupling
        if (amount > payer.getBalance()) {
            ...
            if (result == JOptionPane.YES_OPTION) {
                payer.deductMoney(amount);//rule 3
                user.addMoney(amount);//rule 2
                return new String[] { "Success" };
            } else {
